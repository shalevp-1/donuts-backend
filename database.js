...
export const db = mysql.createConnection({
    ...
    database: process.env.MYSQL_DATABASE || 'donuts',
...
// ✅ FIXED CORS - works for both local and production
app.use(cors({
    origin: process.env.NODE_ENV === 'production' 
        ? process.env.FRONTEND_URL || 'https://your-donut-app.netlify.app'
        : 'http://localhost:3000',
...
app.post("/donuts", (req, res) => {
    ...
    db.query(q, [values], (err, data) => {
        if (err) return res.json(err);
        return res.json("Donut has been created successfully");
    });
});
...
app.delete("/donuts/:id", (req, res) => {
    ...
    db.query(q, [donutId], (err, result) => {
        if (err) return res.json(err);
        return res.json("Donut has been deleted successfully");
    });
});
...
app.put("/donuts/:id", (req, res) => {
    ...
    db.query(q, [...values, donutId], (err, result) => {
        if (err) return res.json(err);
        return res.json("Donut has been updated successfully");
    });
});
...
app.post("/register", (req, res) => {
    bcrypt.hash(req.body.password.toString(), saltRounds, (err, hash) => {
        ...
        const q = "INSERT INTO login (username, email, password, name) VALUES (?)";
        const values = [
            req.body.username,
            req.body.email,
            hash,
    ...
    db.query(q, [req.body.username], (err, result) => {
        ...
        if (result.length > 0) {
            bcrypt.compare(req.body.password.toString(), result[0].password, (err, isMatch) => {
                ...
                if (isMatch) {
                    const token = jwt.sign({ name: result[0].name }, process.env.JWT_SECRET || "jwt-secret-key", { expiresIn: '1h' });
